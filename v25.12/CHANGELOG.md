![Based On](https://img.shields.io/badge/Google-Gemma%203-green)
![Parameter](https://img.shields.io/badge/Parameter-270M-blue)
![Quantized](https://img.shields.io/badge/Quantized%20By-Unsloth-red)

# Previx Neural Engine
Android Local Artificial Inteligence Platform Powered By Google Gemma 3

## v25.12

### Artificial Inteligence Model Information
- **Base Model:** [Google Gemma 3](https://ai.google.dev/gemma) 
- **Parameter:** 270M
- **Quantized By:** [Unsloth AI](https://unsloth.ai/)
- **Type:** Google Gemma 3 270M it UD IQ2 XXS
- **Models Source:** [HuggingFace](https://huggingface.co/unsloth/gemma-3-270m-it-GGUF/blob/main/gemma-3-270m-it-UD-IQ2_XXS.gguf)
- **Status:** Working...

## Credits & Attribution

This application is developed by **[lnpro684](https://github.com/lnpro684/)** as the lead developer.

### Third-Party Licenses

- **Base Model:** This application uses [Google Gemma 3](https://ai.google.dev/gemma), subject to the Gemma Terms of Use.

- **License:** This project is licensed under the Apache License 2.0 - see the [LICENSE](LICENSE) file for details.

If you modify or redistribute this application, you **must** retain the original attribution to the lead developer.
